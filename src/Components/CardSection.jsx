...
const CardSection = () => {
  ...
  if (!cards.length) {
    return (
      <p
        className={`text-center text-xl mt-10 transition-colors duration-500 ${
  ...
  return (
    <div
      className={`h-full grid grid-cols-1  sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 lg:gap-4 py-4 w-full transition-colors duration-500 ${
        darkMode === "dark" ? "bg-[#61dcdf] mb-0" : "bg-white mb-12"
      }`}
    >
      {cards.map((card, index) => (
        <Link
          key={index}
          to={card.link}
          className={`h-150 md:h-100 lg:h-95 group mb-19 flex gap-4 justify-center relative items-center text-center flex-col w-full p-10 xl:p-8 cursor-pointer transition duration-200 ease-in 
            ${
              darkMode === "dark"
                ? "bg-[#fff] text-white"
                : "bg-[#cfcdcd42]/20 hover:bg-white text-gray-900"
            }`}
        >
          <img src={card.imgUrl} className="h-110 w-110" alt={card.title} />
          <div
            className={`flex mt-3 md:mt-1 justify-center gap-1 items-center transition-colors duration-500 group-hover:text-[var(--primaryClr)] ${
